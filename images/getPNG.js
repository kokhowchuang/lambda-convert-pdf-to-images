...
exports.handler = async (event, context) => {
  ...
  const data = await sts.assumeRole({
      ...
      RoleSessionName: 'GonvertPDFtoPNGRole',
    }).promise();
  ...
  for (const record of event.Records) {
    ...
    const rawFileDir = `/tmp/raw/pdftopng/${documentId}`;
    const resultFileDir = `/tmp/result/pdftopng/${documentId}`;
    ...
    const archive = () => {
      return new Promise(async (resolve, reject) => {
        ...
        const convertedFileList = glob.sync(`${resultFileDir}/*.png`);
        let index = 1;

        consoleList['fileList'] = convertedFileList;

        for (let convertedFile of convertedFileList) {
          const shortFileName = `${index.toString().padStart(3, '0')}.png`;
    ...
    const convertFile = (objectKey, keyIndex) => {
      return new Promise(async(resolve, reject) => {
        ...
        s3.getObject(params, function(err, s3Object) {
          ...
          const destinationPath = `${resultFileDir}/${keyIndex}_%03d.png`;
    ...
    try {
      ...
      const jsonFilePath = (record.s3.object.key.indexOf('pdftopng') > -1) ? record.s3.object.key : `pdftopng/${record.s3.object.key}`;

      const targetFileName = jsonFilePath.split('pdftopng/').pop();
